            ...
            <div class="box">
                ...
                <div class="box-header">
                    <div>
                        ...
                        <a class="btn btn-danger btn-sm AjaxButton" data-toggle="tooltip" title="删除选中数据" data-confirm-title="删除确认" data-confirm-content="您确定要删除选中的数据吗？" data-id="checked" data-url="<?php echo url('del'); ?>">
                        ...
                        <a class="btn btn-success btn-sm ReloadButton" data-toggle="tooltip" title="刷新" data-id="checked" data-url="<?php echo url('del'); ?>">
                ...
                <div class="box-body table-responsive">
                    <table class="table table-hover table-bordered datatable" width="100%">
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if(is_array($item['role_text']) || $item['role_text'] instanceof \think\Collection || $item['role_text'] instanceof \think\Paginator): if( count($item['role_text'])==0 ) : echo "" ;else: foreach($item['role_text'] as $key=>$role): ?>
                                <small class="label bg-blue"><?php echo htmlentities($role); ?></small>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </td>
                            <td><?php echo htmlentities($item['status_text']); ?></td>
                            <td class="td-do">
                                <a href="<?php echo url('admin_user/edit',['id'=>$item['id']]); ?>"
                                   class="btn btn-primary btn-xs" title="修改" data-toggle="tooltip">
                                    <i class="fa fa-pencil"></i>
                                </a>

                                <a class="btn btn-danger btn-xs AjaxButton" data-toggle="tooltip" title="删除"  data-id="<?php echo htmlentities($item['id']); ?>" data-confirm-title="删除确认" data-confirm-content='您确定要删除ID为 <span class="text-red"><?php echo htmlentities($item['id']); ?></span> 的数据吗' data-url="<?php echo url('del'); ?>">
                                    <i class="fa fa-trash"></i>
                                </a>

                                <?php if($item['status']==1): ?>
                                <a class="btn btn-warning btn-xs AjaxButton" data-toggle="tooltip" title="禁用"
                                   data-id="<?php echo htmlentities($item['id']); ?>" data-confirm-title="禁用确认"
                                   data-confirm-content='您确定要禁用ID为 <span class="text-red"><?php echo htmlentities($item['id']); ?></span> 的数据吗'
                                   data-url="<?php echo url('disable'); ?>">
                                    <i class="fa fa-circle"></i>
                                </a>
                                <?php else: ?>
                                <a class="btn btn-success btn-xs AjaxButton" data-toggle="tooltip" title="启用"
                                   data-id="<?php echo htmlentities($item['id']); ?>" data-confirm-title="启用确认"
                                   data-confirm-content='您确定要启用ID为 <span class="text-red"><?php echo htmlentities($item['id']); ?></span> 的数据吗'
                                   data-url="<?php echo url('enable'); ?>">
                                    <i class="fa fa-circle"></i>
                                </a>
                                <?php endif; ?>
